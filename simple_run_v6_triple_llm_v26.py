# simple_run_v6_triple_llm_v26.py
"""
EV-IDS-Agent VERSION 6 - TRIPLE LLM V26 Runner
Same interactive flow, scenarios and analysis sections as V23. V26 adds:
...
from ev_ids_agent_v6_triple_llm_v26 import (
    make_llm_client, EVIDSAgentV6TripleLLMV26, build_xai_store,
...
def generate_confusion_heatmaps(results_dict, output_dir):
    ...
    for llm_name, results in results_dict.items():
        all_models_data[f"{llm_name} Agent (V26)"] = [r['predicted'] for r in results]

    n_models = len(all_models_data)
    cols = 4
    rows = (n_models + cols - 1) // cols
    fig, axes = plt.subplots(rows, cols, figsize=(cols * 4, rows * 3.5))
    fig.suptitle('Confusion Matrices — V26 (SHAP+LIME, always-classify)',
    ...
    plt.tight_layout()
    path = os.path.join(output_dir, 'confusion_matrices_v26.png')
...
def print_triple_comparison(results_llama, results_qwen, results_glm, output_dir):
    ...
    print(f"\n{'='*160}")
    print(f"TRIPLE LLM COMPARISON — VERSION 6 V26 (SHAP+LIME, always-classify) | {sc_label}")
    print(f"{'='*160}")
    ...
    # 26/50). Reporting only the all-N number would understate V26 against
    ...
    for llm_name, metrics, results in [
            ('Llama Agent V26', llama_m['IDS_Agent'], results_llama),
            ('Qwen Agent V26',  qwen_m['IDS_Agent'],  results_qwen),
            ('GLM Agent V26',   glm_m['IDS_Agent'],   results_glm)]:
        m    = metrics
        n_fb = sum(1 for r in results if r['result'].get('used_fallback'))
    ...
    # COMPLEXITY
    print(f"\n{'='*120}")
    print(f"COMPLEXITY (V26: bounded generation + shared XAI)")
    print(f"{'='*120}")
    ...
    # FINAL RANKINGS
    print(f"\n{'='*120}")
    print(f"FINAL RANKINGS | {sc_label} | V26 (SHAP+LIME, always-classify)")
    print(f"{'='*120}")
    best_ml_name = max(ml_names, key=lambda m: llama_m[m]['accuracy'])
    all_ranked   = sorted([
        ('GLM Agent V26',      glm_m['IDS_Agent']['accuracy']),
        ('Qwen Agent V26',     qwen_m['IDS_Agent']['accuracy']),
        ('Llama Agent V26',    llama_m['IDS_Agent']['accuracy']),
...
def main():
    print(f"""
========================================================================
  EV-IDS-Agent VERSION 6 — TRIPLE LLM COMPARISON V26
  (built on V23 — same idea, objective, flow and prompts)

  Llama3 | Qwen3.5 | GLM-4.7-Flash

  V26 fixes (performance/robustness only):
...
""")
  ...
  The full V23 per-session trace is printed either way.""")
    ...
    # SELF-CONSISTENCY (V26 accuracy enhancement; pipeline unchanged)
    print("""
============================================================
...
""")
    ...
    bootstrap = EVIDSAgentV6TripleLLMV26(config,
    ...
    def make_agent(model_name):
        ...
        return EVIDSAgentV6TripleLLMV26(
            config, client,
    ...
    results_file = os.path.join(results_dir, f"triple_llm_v6_v26_{scenario_tag}_{ts}.json")
    try:
        with open(results_file, 'w') as f:
            json.dump(make_json_serializable({
                'version':       '6_triple_llm_v26',
        ...
        print(f"\nSave error: {e}")
    print(f"\nTRIPLE LLM V26 SCENARIO {choice} COMPLETE!\n")
