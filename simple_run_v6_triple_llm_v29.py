# simple_run_v6_triple_llm_v29.py
"""
EV-IDS-Agent VERSION 6 - TRIPLE LLM V29 Runner  (final consolidated version)

Pipeline, prompts, 7-step flow, scenarios (A/B/C/D) and the V23-style
explainability output are UNCHANGED. V29 is V23 plus accumulated fixes.

RUN REQUIREMENTS
    All three files must sit in the SAME folder:
        ev_ids_agent_v6_triple_llm_v29.py
        simple_run_v6_triple_llm_v28.py
        llm_eval_metrics_v29.py
...
from llm_eval_metrics_v29 import (
...
from ev_ids_agent_v6_triple_llm_v29 import (
    make_llm_client, EVIDSAgentV6TripleLLMV29, build_xai_store,
    unload_ollama_model,
...
def generate_confusion_heatmaps(results_dict, output_dir):
    ...
    for llm_name, results in results_dict.items():
        all_models_data[f"{llm_name} Agent (V29)"] = [r['predicted'] for r in results]

    n_models = len(all_models_data)
    cols = 4
    rows = (n_models + cols - 1) // cols
    fig, axes = plt.subplots(rows, cols, figsize=(cols * 4, rows * 3.5))
    fig.suptitle('Confusion Matrices — V29 (SHAP+LIME, always-classify)',
    ...
    plt.tight_layout()
    path = os.path.join(output_dir, 'confusion_matrices_v29.png')
...
def print_triple_comparison(results_llama, results_qwen, results_glm, output_dir):
    ...
    print(f"\n{'='*160}")
    print(f"TRIPLE LLM COMPARISON — VERSION 6 V29 (SHAP+LIME, always-classify) | {sc_label}")
    print(f"{'='*160}")
    ...
    for llm_name, metrics, results in [
            ('Llama Agent V29', llama_m['IDS_Agent'], results_llama),
            ('Qwen Agent V29',  qwen_m['IDS_Agent'],  results_qwen),
            ('GLM Agent V29',   glm_m['IDS_Agent'],   results_glm)]:
        m    = metrics
        n_fb = sum(1 for r in results if r['result'].get('used_fallback'))
    ...
    # COMPLEXITY
    print(f"\n{'='*120}")
    print(f"COMPLEXITY (V29: bounded generation + shared XAI)")
    print(f"{'='*120}")
    ...
    # FINAL RANKINGS
    print(f"\n{'='*120}")
    print(f"FINAL RANKINGS | {sc_label} | V29 (SHAP+LIME, always-classify)")
    print(f"{'='*120}")
    best_ml_name = max(ml_names, key=lambda m: llama_m[m]['accuracy'])
    all_ranked   = sorted([
        ('GLM Agent V29',      glm_m['IDS_Agent']['accuracy']),
        ('Qwen Agent V29',     qwen_m['IDS_Agent']['accuracy']),
        ('Llama Agent V29',    llama_m['IDS_Agent']['accuracy']),
...
def run_temperature_sweep(config, backend, base_url, scenario_tag,
    ...
    for slot, model_name in model_ids.items():
        ...
        for t in temps:
            ...
            agent  = EVIDSAgentV6TripleLLMV29(
...
def main():
    print(f"""
========================================================================
  EV-IDS-Agent VERSION 6 — TRIPLE LLM COMPARISON V29
  (built on V23 — same idea, objective, flow and prompts)

  Llama3 | Qwen3.5 | GLM-4.7-Flash

  V29 = V23 pipeline + accumulated fixes:
...
""")
    ...
    bootstrap = EVIDSAgentV6TripleLLMV29(config,
    ...
    def make_agent(model_name):
        ...
        return EVIDSAgentV6TripleLLMV29(
            config, client,
    ...
    ts           = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = os.path.join(results_dir, f"triple_llm_v6_v29_{scenario_tag}_{ts}.json")
    try:
        with open(results_file, 'w') as f:
            json.dump(make_json_serializable({
                'version':       '6_triple_llm_v29',
        ...
        print(f"\nSave error: {e}")
    print(f"\nTRIPLE LLM V29 SCENARIO {choice} COMPLETE!\n")
