...
class OllamaClient:
    ...
    def __init__(self, model_name: str, base_url: str = "http://localhost:11434"):
        ...
        n = model_name.lower()
        self.thinking_capable = ("qwen" in n) or ("glm" in n)
        try:
    ...
    def _call(self, messages, temperature, max_tokens):
        ...
        if self.thinking_capable:
            try:
                ...
                # model rejects think param — remember and retry plain
    ...
    def _extract(self, response, tag="THINK"):
        ...
        # Answer stuck in the thinking channel (V23's GLM failure mode)
...
class XAIExplainer:
    ...
    def __init__(self, models: dict, background_X: np.ndarray,
                 ...
                 kernel_nsamples: int = 200, lime_num_samples: int = 200):
        self.models          = models
        ...
        if SHAP_AVAILABLE and model_name in self.shap_explainers:
            try:
                ...
                if isinstance(explainer, shap.KernelExplainer):
                    raw = explainer.shap_values(X_scaled, nsamples=self.kernel_nsamples,
                                                silent=True)
                else:
...
def build_xai_store(config: dict, df: pd.DataFrame, column_mapping: dict,
    ...
    try:
        ...
        cache_path = os.path.join(
            config.get('workspace_dir', './ev_ids_workspace_v6'), 'xai_cache_v25.pkl')
...
# ══════════════════════════════════════════════════════════════════════════════
class EVIDSAgentV6TripleLLMV25:
    """
...
Provide your structured analysis using the format in your instructions."""
    ...
    def _parse_llm_response(self, stage2_response, stage1_response,
        ...
        return {
            ...
            "aggregation_method":   ("V25_RAG_LTM_XAI" if (self.use_knowledge and self.use_memory)
                                     else "V25_RAG_XAI"  if self.use_knowledge
                                     else "V25_LTM_XAI"  if self.use_memory
                                     else "V25_ML_XAI"),
