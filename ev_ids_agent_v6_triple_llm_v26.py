...
class OllamaClient:
    ...
    def __init__(self, model_name: str, base_url: str = "http://localhost:11434"):
        ...
        # V23 used think=True for Qwen ONLY. Enabling it for GLM changed its
        # output channel and cost accuracy, so V26 restores V23 behaviour.
        self.thinking_capable = "qwen" in model_name.lower()
        try:
    ...
    def _call(self, messages, temperature, max_tokens):
        # Only send options that are explicitly set. Leaving temperature and
        # num_predict unset reproduces V23 exactly (Ollama defaults, unbounded).
...
def make_llm_client(backend: str, model_name: str, base_url: str):
    if backend == 'vllm':
        return VLLMClient(model_name, base_url)
...
def build_xai_store(config: dict, df: pd.DataFrame, column_mapping: dict,
    ...
    try:
        ...
        cache_path = os.path.join(
            config.get('workspace_dir', './ev_ids_workspace_v6'), 'xai_cache_v26.pkl')
...
# ══════════════════════════════════════════════════════════════════════════════
class EVIDSAgentV6TripleLLMV26:
    """
...
Provide your structured analysis using the format in your instructions."""
    ...
    def _parse_llm_response(self, stage2_response, stage1_response,
        ...
        return {
            ...
            "aggregation_method":   ("V26_RAG_LTM_XAI" if (self.use_knowledge and self.use_memory)
                                     else "V26_RAG_XAI"  if self.use_knowledge
                                     else "V26_LTM_XAI"  if self.use_memory
                                     else "V26_ML_XAI"),
