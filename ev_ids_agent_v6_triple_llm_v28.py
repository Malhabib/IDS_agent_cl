...
"""
EV-IDS-Agent VERSION 6 - TRIPLE LLM V28   (final consolidated version)
...
FIX HISTORY FOLDED INTO THIS VERSION
  Generation semantics (V26)
    ...
    tokens) and flattened the reasoning. V28 keeps V23 semantics and uses a
  ...
  Verdict handling (V26)
    ...
    all discarded as ABSTAIN — this is why GLM lost 24/50 sessions. V28
...
REQUIRES (same folder): simple_run_v6_triple_llm_v28.py, llm_eval_metrics_v28.py
...
def build_xai_store(config: dict, df: pd.DataFrame, column_mapping: dict,
    ...
    try:
        ...
        cache_path = os.path.join(
            config.get('workspace_dir', './ev_ids_workspace_v6'), 'xai_cache_v28.pkl')
...
# ══════════════════════════════════════════════════════════════════════════════
class EVIDSAgentV6TripleLLMV28:
    """
...
Provide your structured analysis using the format in your instructions."""
    ...
    def _parse_llm_response(self, stage2_response, stage1_response,
        ...
        return {
            ...
            "aggregation_method":   ("V28_RAG_LTM_XAI" if (self.use_knowledge and self.use_memory)
                                     else "V28_RAG_XAI"  if self.use_knowledge
                                     else "V28_LTM_XAI"  if self.use_memory
                                     else "V28_ML_XAI"),
