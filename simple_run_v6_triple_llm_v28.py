...
"""
EV-IDS-Agent VERSION 6 - TRIPLE LLM V28 Runner  (final consolidated version)

Pipeline, prompts, 7-step flow, scenarios (A/B/C/D) and the V23-style
explainability output are UNCHANGED. V28 is V23 plus accumulated fixes.

RUN REQUIREMENTS
    All three files must sit in the SAME folder:
        ev_ids_agent_v6_triple_llm_v28.py
        simple_run_v6_triple_llm_v28.py
        llm_eval_metrics_v28.py
...
from llm_eval_metrics_v28 import (
...
from ev_ids_agent_v6_triple_llm_v28 import (
    make_llm_client, EVIDSAgentV6TripleLLMV28, build_xai_store,
    unload_ollama_model,
...
def generate_confusion_heatmaps(results_dict, output_dir):
    ...
    for llm_name, results in results_dict.items():
        all_models_data[f"{llm_name} Agent (V28)"] = [r['predicted'] for r in results]

    n_models = len(all_models_data)
    cols = 4
    rows = (n_models + cols - 1) // cols
    fig, axes = plt.subplots(rows, cols, figsize=(cols * 4, rows * 3.5))
    fig.suptitle('Confusion Matrices — V28 (SHAP+LIME, always-classify)',
    ...
    plt.tight_layout()
    path = os.path.join(output_dir, 'confusion_matrices_v28.png')
...
def print_triple_comparison(results_llama, results_qwen, results_glm, output_dir):
    ...
    for llm_name, metrics, results in [
            ('Llama Agent V28', llama_m['IDS_Agent'], results_llama),
            ('Qwen Agent V28',  qwen_m['IDS_Agent'],  results_qwen),
            ('GLM Agent V28',   glm_m['IDS_Agent'],   results_glm)]:
        m    = metrics
        n_fb = sum(1 for r in results if r['result'].get('used_fallback'))
    ...
    # COMPLEXITY
    print(f"\n{'='*120}")
    print(f"COMPLEXITY (V28: bounded generation + shared XAI)")
    print(f"{'='*120}")
    ...
    # FINAL RANKINGS
    print(f"\n{'='*120}")
    print(f"FINAL RANKINGS | {sc_label} | V28 (SHAP+LIME, always-classify)")
    print(f"{'='*120}")
    best_ml_name = max(ml_names, key=lambda m: llama_m[m]['accuracy'])
    all_ranked   = sorted([
        ('GLM Agent V28',      glm_m['IDS_Agent']['accuracy']),
        ('Qwen Agent V28',     qwen_m['IDS_Agent']['accuracy']),
        ('Llama Agent V28',    llama_m['IDS_Agent']['accuracy']),
...
def run_temperature_sweep(config, backend, base_url, scenario_tag,
    ...
    for slot, model_name in model_ids.items():
        ...
        for t in temps:
            ...
            agent  = EVIDSAgentV6TripleLLMV28(
...
def main():
    print(f"""
========================================================================
  EV-IDS-Agent VERSION 6 — TRIPLE LLM COMPARISON V28
  (built on V23 — same idea, objective, flow and prompts)

  Llama3 | Qwen3.5 | GLM-4.7-Flash

  V28 = V23 pipeline + accumulated fixes:
...
""")
    ...
    bootstrap = EVIDSAgentV6TripleLLMV28(config,
    ...
    def make_agent(model_name):
        ...
        return EVIDSAgentV6TripleLLMV28(
            config, client,
    ...
    try:
        ...
        print(f"\nSave error: {e}")
    print(f"\nTRIPLE LLM V28 SCENARIO {choice} COMPLETE!\n")
