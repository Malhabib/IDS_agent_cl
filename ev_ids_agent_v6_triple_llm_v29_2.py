...
"""
EV-IDS-Agent VERSION 6 - TRIPLE LLM V29.2   (final consolidated version)

HEADLINE CHANGE IN V29.2
...
FIX HISTORY FOLDED INTO THIS VERSION
  Generation semantics (V26)
    ...
    tokens) and flattened the reasoning. V29.2 keeps V23 semantics and uses a
  ...
  Verdict handling (V26)
    ...
    all discarded as ABSTAIN — this is why GLM lost 24/50 sessions. V29.2
...
REQUIRES (same folder): simple_run_v6_triple_llm_v28.py, llm_eval_metrics_v29_2.py
...
def build_xai_store(config: dict, df: pd.DataFrame, column_mapping: dict,
    ...
    try:
        ...
        cache_path = os.path.join(
            config.get('workspace_dir', './ev_ids_workspace_v6'), 'xai_cache_v29_2.pkl')
...
# ══════════════════════════════════════════════════════════════════════════════
class EVIDSAgentV6TripleLLMV29_2:
    """
...
Provide your structured analysis using the format in your instructions."""
    ...
    def _parse_llm_response(self, stage2_response, stage1_response,
        ...
        return {
            ...
            "aggregation_method":   ("V29_2_RAG_LTM_XAI" if (self.use_knowledge and self.use_memory)
                                     else "V29_2_RAG_XAI"  if self.use_knowledge
                                     else "V29_2_LTM_XAI"  if self.use_memory
                                     else "V29_2_ML_XAI"),
